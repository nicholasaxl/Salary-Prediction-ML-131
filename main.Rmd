...
```{r, results='hide'}
set.seed(123)
nrow(salaries) #data has 3755 observations
# initial split (70-30)
train = sample(1:nrow(salaries), 2628)
salaries.train = salaries[train, ]
salaries.test = salaries[-train, ]

YTrain = salaries.train$salary_in_usd
XTrain = salaries.train %>% select(-salary_in_usd)

YTest = salaries.test$salary_in_usd
YTest = salaries.test %>% select(-salary_in_usd)
```
### K-NN Regression (one-hot encoding)
```{r}
set.seed(123)
```
...
```
Getting R^2, Mean absolute error and Mean Squared Error for the log response
```{r, results='hide'}
